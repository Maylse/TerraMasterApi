...
public function getExperts(Request $request): JsonResponse
{
    ...
    $experts = User::where('user_type', 'expert')
        ->with('expert') // Eager load the expert relationship
        ->get();
    ...
    $formattedExperts = $experts->map(function($expert) {
        return [
            ...
            'name' => $expert->name,
            'email' => $expert->email,
            'certification_id' => $expert->expert->certification_id ?? null,
            'license_number' => $expert->expert->license_number ?? null,
            'pricing' => $expert->expert->pricing ?? null,
        ];
    });
